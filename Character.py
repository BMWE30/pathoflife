...
class Character:
    ...
    def __init__(self, variant, xy, keys, textures, sounds, fonts):
        ...
        self.mentalhearts = [UIMentalHealth(
            0, 2, textures["mental"]) for i in range(3)]
        ...
        self.feet = [
            [self.textures.subsurface(Rect((i*64), 64, 64, 64))
             for i in range(8)],
            [self.textures.subsurface(Rect((i*64), 64*2, 64, 64))
             for i in range(8)],
            [],
            []
        ]
        ...
        self.pickups = [Pickup(i, textures["pickups"], fonts["pickups"])
                        for i in range(3)]  # Keys, Bombs, Coins
    ...
    def usePill(self):
        if self.pill != None:  # Ensure the character has a pill
            ...
            if sum(st) == -1:
                # Its a negative pill
                self.game.banners.append(
                    Banner(types[st.index(-1)] + " Down", self.game.textures))
            else:
                # Its a positive pill
                self.game.banners.append(
                    Banner(types[st.index(1)] + " Up", self.game.textures))
    ...
    def render(self, surface, time, bounds, obsticals, doors):
        ...
        if sum(map(int, [self.left, self.right, self.up, self.down])) > 1:
            # So there is no benefit to going diagonal (sqrt(2))
            sizeModifier /= 1.414213
            ...
            if rcx or rcy:
                if type(ob) == Fire:
                    ...
                    if self.pickups[0].use(ob.price):
                        ...
                        if amm == 0:
                            self.hearts.append(
                                UIHeart(ob.variant, ob.health, self.heartTextures))
                        elif type(amm) == int:
                            self.hearts.append(
                                UIHeart(ob.variant, amm, self.heartTextures))
                        ob.pickup()
        ...
        if self.specialFrame == 0:
            ...
            surface.blit(
                self.specialFrames[self.specialFrame-1], (self.x-64, self.y-72))
