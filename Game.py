...
class Game:
    ...
    def setup(self):
        ...
        floor = loadFloor(["basement.xml", "basement.xml", "basement.xml", "basement.xml", "basement.xml", "basement.xml",
                                           "basement.xml"][self.floorIndex], self.floorIndex, randint(8, 12), self.sounds, self.textures)
        ...
        self.floor = floor
        self.banners.append(Banner(["Birth", "Caves", "Catacombs", "Necropolis",
                                    "Depths", "Womb", "Uterus"][self.floorIndex], self.textures))
    ...
    def run(self, screen, sounds, textures, fonts, joystick=None):
        ...
        cn = self.controls
        self.isaac = isaac = Character(self.characterType, (WIDTH//2, (HEIGHT//4)*3), [
            [cn[3], cn[1], cn[2], cn[0]], [cn[7], cn[5], cn[6], cn[4]]], textures, sounds, fonts)
        ...
        # Create minimap
        self.minimap = Surface((textures["map"]["background"].get_width(
        ), textures["map"]["background"].get_height())).convert_alpha()
        self.updateMinimap(self.currentRoom)
        ...
        while running:
                ...
                if e.type == QUIT:
                    ...
                    if e.key == self.controls[-1]:
                        ...
                        if isaac.pickups[1].use(1):
                            self.floor[self.currentRoom].other.append(Bomb(self.floor[self.currentRoom], 0, ((
                                isaac.x-GRIDX)/GRATIO, (isaac.y-GRIDY)/GRATIO), [sounds["explosion"]], textures["bombs"], explode=True))
            ...
            if len(animatingRooms) > 0:
                ...
                # Render the room
                move = self.floor[self.currentRoom].render(
                    screen, isaac, currTime)

                if move[0] != 0 or move[1] != 0:
                    old = tuple(self.currentRoom[:])

                    self.currentRoom = (
                        move[0]+self.currentRoom[0], move[1]+self.currentRoom[1])
                    try:
                        # Animate the room
                        ...
                        if self.floor[self.currentRoom].variant == 2 and not self.floor[self.currentRoom].entered:
                            ...
                            bossIntro(screen, self.characterType, [Gurdy, Duke].index(
                                type(self.floor[self.currentRoom].enemies[0])), self.floorIndex)
            ...
            if self.floor[self.currentRoom].variant == 2:
                ...
                try:
                    # Draw the boss bar
                    bossbar(
                        screen, self.floor[self.currentRoom].enemies[0].health/100)
                except:
                    pass
